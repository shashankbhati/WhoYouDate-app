...
import { seedEntries, seedEntriesIndia, seedEntriesUS, seedEntriesDresden, seedEntriesTabeaShashank, seedEntriesLeoni, seedPosts } from "./seed";
...
async function initialize() {
  ...
  try {
    ...
    if (await shouldSeedCountry("seeded_leoni")) {
      const seeds = seedEntriesLeoni();
      for (let i = 0; i < seeds.length; i += 100) {
        await supabase.from("entries").insert(seeds.slice(i, i + 100).map(entryToRow));
      }
      seeded = true;
